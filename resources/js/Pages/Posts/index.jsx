...
const Posts = () => {
	...
	return (
        ...
        >
            <Head title="Posts" />
            <Breadcrumb title="Posts" subtitle="Posts List" />

            <div className="py-6">
<div className="card">
    <div className="card-header">
        <h6 className="card-title">Posts List</h6>
        <Link href="/dashboard/posts/create" className="btn btn-sm bg-primary text-white">
    ...
    <div className="flex flex-col">
                    ...
                    <table className="min-w-full divide-y divide-default-200">
                        <thead className="bg-default-150">
                            <tr className="text-sm font-normal text-default-700 whitespace-nowrap">
                                ...
                                <th className="px-3.5 py-3 text-start">Headline</th>
                        ...
                        <tbody>
                            {posts.map(post => <tr key={post.id} className="text-default-800 font-normal text-sm whitespace-nowrap">
                                <td className="py-3 ps-4">
                                    <input type="checkbox" className="form-checkbox" />
                            </td>
                            <td className="px-3.5 py-3 text-primary">{post.id}</td>
                        <td className="py-3 px-3.5">{post.category_id}</td>
                        <td className="py-3 px-3.5">{post.title}</td>
                        <td className="py-3 px-3.5">{post.headline}</td>
                        <td className="py-3 px-3.5">{post.active}</td>
                        <td className="py-3 px-3.5">{post.created_by}</td>
                        <td className="py-3 px-3.5">{post.created_at}</td>
                        <td className="px-3.5 py-3">
                            <div className="hs-dropdown relative inline-flex">
                                ...
                                <div className="hs-dropdown-menu" role="menu">
                                    <Link href={`/dashboard/posts/${post.id}`} className="flex items-center gap-1.5 py-1.5 px-3 text-default-500 hover:bg-default-150 rounded">
        ...
        <div className="card-footer">
            <p className="text-default-500 text-sm">
                Showing <b>{posts.length}</b> of <b>{posts.length}</b> Results
            </p>
                                        ...
                                        <tbody>
                                        {
											posts.map((post, i) => (
	                                            <tr key={i} className="odd:bg-white even:bg-gray-100 hover:bg-gray-100 dark:odd:bg-neutral-800 dark:even:bg-neutral-700 dark:hover:bg-neutral-700">
	                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">{post.category_id}</td>
	                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{post.title}</td>
	                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{post.headline === 'Y' ? 'Yes' : 'No'}</td>
