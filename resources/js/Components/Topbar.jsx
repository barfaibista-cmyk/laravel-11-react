...
import { TbSearch, TbMoon, TbSun } from 'react-icons/tb';
import { showBackdrop, toggleClassName } from '@/Utils/layout';
import { useLayoutContext } from '@/Context/useLayoutContext';
import { LuAlignLeft, LuBellRing, LuClock, LuGem, LuHeart, LuLogOut, LuMail, LuMessagesSquare, LuMoveRight, LuSettings, LuShoppingBag } from 'react-icons/lu';
...
export default function Topbar(props) {
	const { sidenav, theme, updateSettings } = useLayoutContext();
	const { size } = sidenav;
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="topbar-item hs-dropdown [--auto-close:inside] relative inline-flex">
                        ...
                        <div className="flex items-center justify-between p-4 border-t border-default-200">
                            <Link to="#!" className="text-sm font-medium text-default-900">Manage Notification </Link>
                            <button type="button" className="btn btn-sm text-white bg-primary">View All <LuMoveRight className="size-4" />
                            </button>
                        </div>
                    </div>
				</div>
                    ...
                    <div className="topbar-item hs-dropdown relative inline-flex">
                        ...
                        <div className="hs-dropdown-menu min-w-48">
							...
							<div className="flex flex-col gap-y-1">
								<Link href="/profile" className="flex items-center gap-x-3.5 py-1.5 px-3 text-default-600 hover:bg-default-150 rounded font-medium">
									Profile
								</Link>
								<Link method="POST" href={route('logout')} className="flex items-center gap-x-3.5 py-1.5 px-3 text-default-600 hover:bg-default-150 rounded font-medium">
									Logout
